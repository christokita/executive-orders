...
#' @description This is the main document term matrix creating function for IDA's text mining R package.
...
Vec2Dtm <- function(vec, min.n.gram=1, max.n.gram=2, remove.stopwords=TRUE, custom.stopwords=NULL, lower=TRUE, remove.punctuation=TRUE, remove.numbers=TRUE, stem.document=FALSE){
	...
	if(remove.stopwords){
	    message("Creating combined stop words list...")
  ...
  docnames <- names(vec)
	...
	if( lower ) {
	    message("Converting all terms to lower case...")
        vec <- tolower(vec)
	}
