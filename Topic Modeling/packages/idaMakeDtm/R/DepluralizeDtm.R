...
DepluralizeDtm <- function(dtm){
    ...
    message("Attempting to normalize plural words to singular form...")
    ...
    message("splitting dtm into columns not changed and columns that need to be merged...")
    ...
    message("Determining which columns should be merged together...")
...
#     merge.list <- lapply(term.indices, function(y) changed[ , y ])
    ...
    message("Computing merged column summed frequency counts")
    sfInit(parallel=TRUE, cpus=4)
	sfExport("changed")
	sfLibrary(Matrix)
	...
	message("Preparing a sparse matrix representation of the dtm...")
    ...
    message("Done depluralizing dtm.")
	return(dtm)
